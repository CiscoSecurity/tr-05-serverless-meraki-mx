...
from requests.exceptions import ConnectionError, HTTPError, InvalidURL, SSLError

from api.errors import AuthorizationError, InvalidArgumentError, MerakiMXConnectionError, MerakiMXSSLError
...
WRONG_KEY = (
    ...
    "Make sure domain in custom_jwks_host corresponds to your SecureX instance region."
)
JWKS_HOST_MISSING = "jwks_host is missing in JWT payload. Make sure custom_jwks_host field is present in module_type"
WRONG_JWKS_HOST = "Wrong jwks_host in JWT payload. Make sure domain follows the visibility.<region>.cisco.com structure"
...
def get_public_key(jwks_host, token):
    ...
    expected_errors = (ConnectionError, InvalidURL, KeyError, JSONDecodeError, HTTPError)
    try:
...
def query_sightings(observables, credentials):
    ...
    appliance_events_url = (
        f"https://api.meraki.com/api/v1/organizations/{org_id}"
        f"/appliance/security/events?sortOrder=descending&perPage={entities_limit}"
    )
    ...
    nbar_events_url = (
        ...
        f"?productType=appliance&includedEventTypes%%5B%%5D=nbar_block&perPage={entities_limit}"
    )
    cf_events_url = (
        f"https://api.meraki.com/api/v1/networks/{network_id}/events"
        f"?productType=appliance&includedEventTypes%%5B%%5D=cf_block&perPage={entities_limit}"
    )
    ...
    for record in json_result:
        ...
        if event_type and event_type == "IDS Alert":
            ...
            if indicator in dst_ip or indicator in src_ip or indicator in device_mac or indicator in client_mac:
            ...
            if (
                ...
                or indicator in canonical_name
            ):
                results.append(record)
...
def get_jwt():
    ...
    try:
        ...
        payload = jwt.decode(token, key, ["RS256"], audience=[aud.rstrip("/")])
