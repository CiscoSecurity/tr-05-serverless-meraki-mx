...
from api.utils import get_client_info, get_device_info, get_ip, parse_rule_id_to_snort_link
    ...
    def get_relations(event):
        ...
        if event_type == "IDS Alert":
            if event.get("srcIp") and event.get("destIp") and event["srcIp"] != event["destIp"]:
                relations.append(
                    {
                        "related": {"type": "ip", "value": get_ip(event["destIp"])},
                        "source": {"type": "ip", "value": get_ip(event["srcIp"])},
            ...
            if event.get("deviceMac") and event.get("srcIp"):
                relations.append(
                    {
                        "related": {"type": "mac_address", "value": event["deviceMac"]},
                        "source": {"type": "ip", "value": get_ip(event["srcIp"])},
                        "origin": "Meraki Module",
                        "relation": "Targeted",
                    }
        ...
        if event_type == "File Scanned":
            ...
            output_downloaded_by = {
                ...
                "source": {"value": file_hash, "type": "sha256"},
                "related": {"value": get_ip(event["srcIp"]), "type": "ip"},
            }
            output_downloaded_from = {
                "origin": "Meraki MX",
                "relation": "Downloaded_From",
                "source": {"value": get_ip(event["destIp"]), "type": "ip"},
                "related": {"value": file_hash, "type": "sha256"},
            }
            output_filename_of = {
                "origin": "Meraki MX",
                "relation": "Filename_Of",
                "source": {"value": event.get("canonicalName"), "type": "file_name"},
                "related": {"value": file_hash, "type": "sha256"},
            }
            ...
            if (event["eventData"].get("Source IP") and event["eventData"].get("Destination IP")) and (
                ...
                relations.append(
                    {
                        "related": {"type": "ip", "value": get_ip(event["eventData"]["Destination IP"])},
                        "source": {"type": "ip", "value": get_ip(event["eventData"]["Source IP"])},
                        "origin": "Meraki Module",
            ...
            relations.append(
                {
                    "related": {"type": "ip", "value": get_ip(event["eventData"]["server"])},
                    "source": {"type": "mac_address", "value": event["clientMac"]},
                    "origin": "Meraki Module",
                    "relation": "Attempted Connection",
                }
            )
            relations.append(
                {
                    "related": {"type": "ip", "value": get_ip(event["eventData"]["server"])},
                    "source": {"type": "url", "value": event["eventData"]["url"]},
                    "origin": "Meraki Module",
                    "relation": "Hosted By",
                }
            )
    ...
    def sighting(self, observable, event):
        ...
        if event_type == "IDS Alert":
            d = {
                ...
                "short_description": "%s" % event["message"],
