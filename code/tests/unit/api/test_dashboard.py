...
def wrong_calls():
    yield WrongCall(
        "/tiles/tile",
        {"tile-id": "some_value"},
        "{'tile_id': ['Missing data for required field.'], " "'tile-id': ['Unknown field.']}",
    )
    yield WrongCall("/tiles/tile", {"tile_id": ""}, "{'tile_id': ['Field may not be blank.']}")
    yield WrongCall(
        "/tiles/tile-data",
    ...
    yield WrongCall(
        "/tiles/tile-data", {"tile_id": "", "period": "some_period"}, "{'tile_id': ['Field may not be blank.']}"
    )
    yield WrongCall(
        "/tiles/tile-data",
        {"tile_id": "some_value", "not_period": "some_period"},
        "{'period': ['Missing data for required field.'], " "'not_period': ['Unknown field.']}",
    )
    yield WrongCall(
        "/tiles/tile-data", {"tile_id": "some_value", "period": ""}, "{'period': ['Field may not be blank.']}"
    )
...
def test_dashboard_call_with_wrong_payload(
    ...
    response = client.post(path=wrong_call.endpoint, headers=get_headers(valid_jwt()), json=wrong_call.payload)
